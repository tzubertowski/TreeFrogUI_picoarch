...
int menu_text_color = 0xfffe; // default to white
int menu_sel_color = -1; // disabled
int menu_sel_text_color = 0x0000; // selected-row text (default black on pill)
...
#else
/* The picoarch menu renders at a fixed 320x240 buffer that is then HW-upscaled
 * ~2x to the panel, so menu px ≈ half of the on-panel size. These match FrogUI's
 * ~30px font / 36px rows after upscale. Bitmap font is fallback only. */
static const int me_mfont_w = 8, me_mfont_h = 18;
static const int me_sfont_w = 6, me_sfont_h = 12;
#endif

/* Menu TTF size in the 320x240 buffer (≈ FrogUI 30px after the ~2x upscale). */
#define MENU_TTF_PX 14
...
static void text_out16_(int x, int y, const char *text, int color)
{
	...
	/* TTF path (matches FrogUI). Skip the (void*)1 selector-symbol case — that's
	 * handled by the bitmap path / menu_draw_selection. */
	if (menu_font_ready() && text != (void *)1) {
		char tmp[256];
		const char *p;
		for (p = text; *p != 0 && *p != '\n'; p++)
			;
		...
		memcpy(tmp, text, len); tmp[len] = 0;
		/* vertically center the glyph cell within the row */
		menu_font_draw_text((uint16_t *)g_menuscreen_ptr, g_menuscreen_w, g_menuscreen_h,
		                    x, y + (me_mfont_h - MENU_TTF_PX) / 2, tmp, (uint16_t)color);
		i = menu_font_measure(tmp);
		if (x < border_left) border_left = x;
		if (x + i > border_right) border_right = x + i;
		if (y < border_top) border_top = y;
		if (y + me_mfont_h > border_bottom) border_bottom = y + me_mfont_h;
		return;
	}
...
/* Filled rounded rect into g_menuscreen (RGB565). */
static void menu_round_fill(int x, int y, int w, int h, int r, unsigned short c)
{
	...
	for (int j = 0; j < h; j++) {
		int yy = y + j;
		if (yy < 0 || yy >= g_menuscreen_h) continue;
		int inset = 0;
		if (j < r)            { int dy = r - j;         inset = r - (int)isqrtf_(r * r - dy * dy); }
		else if (j >= h - r)  { int dy = j - (h - r) + 1; inset = r - (int)isqrtf_(r * r - dy * dy); }
		int xs = x + inset, xe = x + w - inset;
		if (xs < 0) xs = 0;
		if (xe > g_menuscreen_w) xe = g_menuscreen_w;
		unsigned short *row = fb + (size_t)yy * g_menuscreen_pp;
...
static void menu_draw_selection(int x, int y, int w)
{
	...
	/* TTF look: white rounded pill (FrogUI select_bg). Selected text is drawn
	 * black by me_draw. The passed width `w` is computed from the bitmap font
	 * (8px/char) and is too narrow for proportional TTF glyphs — long entries
	 * would overflow the pill and render black-on-dark (invisible). Extend the
	 * pill to the screen edge so any entry stays on the white. */
	if (menu_font_ready()) {
...
void menu_init_base(void)
{
	...
	if (f != NULL)
	{
		...
		while (!feof(f))
		{
			...
			else if (strncmp(buff, "sel_text_color=", 15) == 0)
			{
				int tmp = parse_hex_color(buff+15);
				if (tmp >= 0) menu_sel_text_color = tmp;
				else lprintf("skin.txt: parse error for sel_text_color\n");
			}
			else
...
/* MinUI-style: slot list on the left, selected slot's screenshot on the right. */
static void draw_savestate_menu(int menu_sel, int is_loading)
{
	...
	if (!menu_font_ready()) {
		...
		for (i = 0; i < STATE_SLOT_COUNT; i++, y += me_mfont_h) {
			if (!(state_slot_flags & (1 << i))) strcpy(time_buf, "free");
			else {
				strcpy(time_buf, "USED");
				if (state_slot_times[i]) { time_t t = state_slot_times[i]; strftime(time_buf, sizeof(time_buf), "%x %R", localtime(&t)); }
			}
	...
	int pad = W / 20;
	int rh  = me_mfont_h;                   /* 36 */
	int tdy = (rh - MENU_TTF_PX) / 2;
	int tw  = (W * 42) / 100, th = (tw * 3) / 4;
	int tx  = W - tw - pad, ty = pad + rh + rh / 2;
	int barw = tx - (pad - 6) - pad / 2;

	menu_draw_begin(1, 0);                 /* darkened game frame bg */

	menu_font_draw_text((uint16_t *)g_menuscreen_ptr, W, H, pad, pad + tdy,
	                    is_loading ? "Load State" : "Save State", WHITE);

	/* selected slot's screenshot thumbnail (bright) */
	int shown = 0;
	if (menu_sel < STATE_SLOT_COUNT && (state_slot_flags & (1 << menu_sel))) {
		static uint16_t *ssbuf = NULL; static size_t sscap = 0;
		size_t need = (size_t)W * H;
		if (need > sscap) { free(ssbuf); sscap = need; ssbuf = (uint16_t *)malloc(sscap * 2); }
		if (ssbuf) {
			char fn[MAX_PATH]; int sw = 0, sh = 0, sbpp = 0;
			state_file_name(fn, sizeof(fn), menu_sel);
			if (plat_load_screen(fn, ssbuf, sscap * 2, &sw, &sh, &sbpp) == 0 && sbpp == 2 && sw > 0) {
				minui_thumb(ssbuf, sw, sh, sw, tx, ty, tw, th);
				shown = 1;
			}
		}
	}
	if (!shown) minui_fill(tx, ty, tw, th, 0x2104);
	minui_border(tx - 3, ty - 3, tw + 6, th + 6, 3, WHITE);

	/* slot rows + Back */
	y = pad + rh * 2;
	for (i = 0; i <= STATE_SLOT_COUNT; i++, y += rh) {
		if (i == STATE_SLOT_COUNT) {
			snprintf(buf, sizeof(buf), "Back");
		} else if (!(state_slot_flags & (1 << i))) {
			snprintf(buf, sizeof(buf), "Slot %d  -  Empty", i + 1);
		} else {
			char tb[24] = "Used";
			if (state_slot_times[i]) { time_t t = state_slot_times[i]; strftime(tb, sizeof(tb), "%x %R", localtime(&t)); }
			snprintf(buf, sizeof(buf), "Slot %d  -  %s", i + 1, tb);
		}
		if (i == menu_sel) menu_round_fill(pad - 6, y, barw, rh, rh / 4, WHITE);
		menu_font_draw_text((uint16_t *)g_menuscreen_ptr, W, H, pad, y + tdy, buf,
		                    (i == menu_sel) ? DARK : WHITE);
	}

	const char *leg = "B-BACK   A-OKAY";
	menu_font_draw_text((uint16_t *)g_menuscreen_ptr, W, H,
	                    W - menu_font_measure(leg) - pad, H - rh + tdy, leg, WHITE);
