 ...
 * The runtime library (1.2.x) doesn't have it — provide compat stub. */
#if PNG_LIBPNG_VER < 10400
/* already handled by old png_jmpbuf */
#else
#undef png_jmpbuf
#define png_jmpbuf(png_ptr) (*((jmp_buf*)((png_ptr))))
#endif
#include "lprintf.h"
...
int readpng(void *dest, const char *fname, readpng_what what, int req_w, int req_h)
{
	...
	png_ptr = png_create_read_struct("1.2.59", NULL, NULL, NULL);
	if (!png_ptr)
	{
...
int writepng(const char *fname, unsigned short *src, int w, int h)
{
	...
	/* initialize stuff */
	png_ptr = png_create_write_struct("1.2.59", NULL, NULL, NULL);
	if (png_ptr == NULL) {
